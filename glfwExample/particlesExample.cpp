// simple glfw 3d example
//{{{  simple particle engine with threaded physics
// Copyright (c) Marcus Geelnard (c) Camilla Löwy <elmindreda@glfw.org>
//
...
#define BATCH_PARTICLES 70  // Number of particles in batch (70 corresponds to 7.5 KB = will not blow L1 cache
...
static void draw_particles (GLFWwindow* window, double t, float dt) {
  ...
  for (i = 0;  i < MAX_PARTICLES;  i++) {
    if (pptr->active) {
      // Calculate particle intensity (we set it to max during 75% of its life, then it fades out)
      alpha =  4.f * pptr->life;
      if (alpha > 1.f)
        alpha = 1.f;

      // Convert color from float to 8-bit (store it in a 32-bit integer using endian independent type casting)
    ...
    // If we have filled up one batch of particles, draw it as a set of quads using glDrawArrays.
